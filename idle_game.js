...
const SMITHING_RECIPES = {
  'Iron Bar': {
    ...
    duration: 3000,             // can override skill duration if you want
    xp: 15,
    output: { name: 'Iron Bar', qty: 1, rarity: 'common' }
  }
};
  ...
  showMainMenu();
    ...
    function selectType(type, btn) {
      ...
      if (currentSkill === 'smithing') {
        ...
        const d = skills[currentSkill];
        const dur = (rec?.duration ?? d.duration) / 1000;
...
const recipeMap = skills[skill]?.consumes;
const recipe = recipeMap?.[type];
if (recipe && runs !== Infinity) {
  const cap = maxCraftable(recipe);
  runs = Math.max(0, Math.min(runs, cap));
  if (runs === 0) {
    // nothing craftable -> exit gracefully
    running = false;
    updateQueueStatus();
    processQueue();
    return;
      ...
      (function iter() {
        ...
        // If smithing, check & consume inputs up-front
        if (skill === 'smithing') {
          const rec = SMITHING_RECIPES[type];
          if (!rec) {
          ...
          // check mats
          const lacking = Object.entries(rec.inputs).filter(([n, q]) => (inventory.loot[n] || 0) < q);
          if (lacking.length) {
            alert('Nicht genug Materialien: ' + lacking.map(([n, q]) => `${q}× ${n}`).join(', '));
            running = false;
          ...
          // consume
          Object.entries(rec.inputs).forEach(([n, q]) => {
            inventory.loot[n] = (inventory.loot[n] || 0) - q;
            if (inventory.loot[n] <= 0) delete inventory.loot[n];
          });
          renderInventory();
          requestSave?.();
        }
...
if (drop) {
  addItem('loot', drop.name, drop.qty, drop.rarity);
            ...
            if (skill === 'smithing') {
              const rec = SMITHING_RECIPES[type];
              // add outputs
              Object.entries(rec.outputs).forEach(([n, q]) => {
                // if you have rollDrop/DROPS for smithing, keep that;
                // else add as common:
                if (typeof rollDrop === 'function' && window.DROPS?.smithing) {
                  // Let rollDrop drive rarity if you want; otherwise:
                  addItem('loot', n, q, 'common');
                } else {
                  addItem('loot', n, q, 'common');
                }
              });
              ...
              // keep your existing resource drop
              const drop = (typeof rollDrop === 'function') ? rollDrop(skill, type) : null;
